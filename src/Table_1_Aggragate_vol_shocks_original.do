/*******************************************************************************
Replication and extension of Table VI in Ang et al. (2006) - Part 2:

Author: Prof. Dr. Alexander Hillert, Leibniz Institute for Financial Research SAFE
and Goethe-University Frankfurt

This version: September 25, 2024
* ssc install outtable

In this do-file, we will replicate Table VI of Ang et al. (2006) and run a 	
robustness check (out-of-sample) test.

The generate structure is that:
1. Open CRSP monthly and merge it with the IVol data from Part 1
2. Sort stocks into five quintile portfolios
3. Compute (value-weighted) portfolio returns
4. Merge with Fama and French monthly factor data
5. Determine the average portfolio returns and alphas

The necessary data sets are:
1. CRSP_monthly_1926_2024.dta
2. iv_vol_monthly_stocks_1926-2024.dta (the result from part 1)
3. FF3_monthly_1926_2024.dta
...
*merge 1:1 permno month_id using DATA_b_deltavxo_monthly_stocks_1986-2001.dta
*merge 1:1 permno month_id using DATA_b_delta_vix_monthly_stocks_1990-2020.dta
...
local ivol_measure="b_d_vix"

keep permno month_id `ivol_measure'

* Portfolio formation
forvalues i=20(20)80 {
  by month_id, sort: egen ivol_percentile_`i'=pctile(`ivol_measure'), p(`i')
}
...
save `ivol_measure'_value_weighted_returns_daily.dta, replace
...
estout using `ivol_measure'_Ang_Table_IV_alphas.txt, cells(b(fmt(4) star) t(fmt(2))) /*
...
merge 1:1 permno month_id using DATA_b_fvix_monthly_stocks_1990-2024_.dta
keep if _merge==3 
drop _merge

collapse (mean) `ivol_measure' (mean) b_fvix, by(ivol_quintile)
...
save `ivol_measure'_avg_betas.dta, replace
...
// size
*********************************/
*use COMPUSTAT_bm_1970-01-31_2024-12-31, clear
*generate month_id=mofd(public_date)
*keep permno bm month_id
*save COMPUSTAT_bm_1970-01-31_2024-12-31.dta, replace
...
outsheet using `ivol_measure'_Ang_Table_IV.txt, replace
