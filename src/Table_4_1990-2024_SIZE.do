...
local sample_end = mofd(mdy(1,1,2024))
...
estout using table_4_`control_filter'_1990_2024_Ang_Table_IV_alphas.txt, cells(b(fmt(4) star) t(fmt(2))) /*
...
collapse (mean) b_d_vix (mean) b_fvix, by(ivol_quintile)
rename `ivol_measure' avg_`ivol_measure'
rename b_fvix avg_b_fvix

label variable avg_`ivol_measure' "Average b_d_vix Ratio per portfolio"
...
outsheet using table_4_`control_filter'_1990_2024_Ang_Table_IV.txt, replace
